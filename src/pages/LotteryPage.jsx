...
function LotteryPage() {
  ...
  const updateBetDetails = useCallback(async () => {
    try {
      ...
      setBet(updatedBet);
      setSegments(updatedBet.option_desc);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!isAfterEndDate) {
      setPublishButtonText(
        `Wheel spins at ${formatDate(bet.end_date)} ${bet.end_time} UTC`
      );
    } else if (!hasEnoughParticipants) {
      setPublishButtonText(
        'Unable to spin wheel (not enough participants joined).'
      );
    } else if (hasPublished) {
      setPublishButtonText('Wheel already spinned.');
    } else {
      setPublishButtonText('NO TEXT - Publish bet');
    }
  }, [
    bet,
    connected,
